...
class TeamIcon(private val ownerInventory : InteractiveInventory,private val associatedTeam : Team, private val totalTaskAmount : Int?){
    private val publicTeamIcon = TTUtil.itemFactory(
            TTUtil.colorMaterial(associatedTeam.color, Material.WHITE_STAINED_GLASS), associatedTeam.getDisplayName(), null)
    private val privateTeamIcon = TTUtil.itemFactory(
            TTUtil.colorMaterial(associatedTeam.color, Material.WHITE_CONCRETE), "${associatedTeam.color}This Card", null, true)
    ...
    fun setClickable(value : Boolean) {
        ...
        clickable = value
        if(value) publicTeamIcon.type = TTUtil.colorMaterial(associatedTeam.color, Material.WHITE_CONCRETE)
        else publicTeamIcon.type = TTUtil.colorMaterial(associatedTeam.color, Material.WHITE_STAINED_GLASS)
        updateIcon()
    }
