...
import me.wanttobee.tasktussle.systems.games.taskTussle.base.ICard
import me.wanttobee.tasktussle.systems.games.taskTussle.base.ICardManager
import me.wanttobee.tasktussle.systems.games.taskTussle.base.ITask
import me.wanttobee.tasktussle.systems.games.taskTussle.tasks.ObtainTask
import me.wanttobee.tasktussle.systems.games.taskTussle.base.TaskState
    ...
    override fun onTaskDisabled(task: ITask) {
        ...
        if(TaskTussleSystem.hideCard){
            BingoGameSystem.game?.applyToTeams{ team,cm ->
                if(cm == this)
                    team.applyToMembers { p -> p.sendMessage("\"${associatedTeam.getDisplayName()}${ChatColor.RESET} got a task ${ChatColor.DARK_GRAY}(${TTUtil.getRealName(obtainTask.itemToObtain)})") }
                else
                    team.applyToMembers { p -> p.sendMessage("\"${associatedTeam.getDisplayName()}${ChatColor.RESET} got a task")}
            }
        }
        else{
            BingoGameSystem.game?.applyToAllMembers { p ->  p.sendMessage("${associatedTeam.getDisplayName()}${ChatColor.RESET} got task: ${ChatColor.GOLD}${TTUtil.getRealName(obtainTask.itemToObtain)}")  }
        }
