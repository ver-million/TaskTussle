...
object TeamSystem2 {
    ...
    fun <T> makeXTeams(setSize : Int, defaultValues : () -> T) : TeamSet<T> {return makeXTeams(setSize, "Unknown Set", null, defaultValues)}
    fun <T> makeXTeams(setSize : Int, title : String , defaultValues : () -> T) : TeamSet<T>{ return makeXTeams(setSize,title, null, defaultValues )}
    fun <T> makeXTeams(setSize : Int, title : String , preGeneratedValues : Array<T>?, defaultValues : () -> T ) : TeamSet<T>{
    ...
    fun <T> teamMaker(sender:Player,defaultValues : () -> T, setSize: Int, title:String, preGeneratedValues : Array<T>?, effect: (TeamSet<T>) -> Unit) {return teamMaker(sender,defaultValues,setSize,title,64,preGeneratedValues,effect)}
    fun <T> teamMaker(sender:Player,defaultValues : () -> T, setSize: Int, title:String, effect:(TeamSet<T>) -> Unit) {return teamMaker(sender,defaultValues, setSize,title,64,null,effect)}
    fun <T> teamMaker(sender:Player,defaultValues : () -> T, setSize: Int, effect: (TeamSet<T>) -> Unit){return teamMaker(sender,defaultValues, setSize, "Unknown Set", 64,null, effect)}
    fun <T> teamMaker(sender:Player,defaultValues : () -> T, setSize: Int, title : String, maxTeamSize : Int, preGeneratedValues : Array<T>?, effect : (TeamSet<T>) -> Unit){
        if(setSize == 1){
